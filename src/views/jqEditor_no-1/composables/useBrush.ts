...
import type { BrushConfig, BrushMetrics, AffectedCell, BrushPreview } from '../types/brush'
...
export function useBrush(gridData: GridData) {
  ...
  const brushMetrics = computed<BrushMetrics>(() => {
    ...
    return {
      ...
      radiusInGrids: Math.ceil(brushSizePx / Math.min(gridData.cellWidth, gridData.cellHeight) / 2),
  ...
  const getAffectedCells = (
    ...
    if (shape === 'circle') {
      ...
      for (let x = centerX - radiusInGrids; x <= centerX + radiusInGrids; x++) {
        for (let y = centerY - radiusInGrids; y <= centerY + radiusInGrids; y++) {
      ...
      for (let x = centerX - halfWidth; x <= centerX + halfWidth; x++) {
        for (let y = centerY - halfHeight; y <= centerY + halfHeight; y++) {
          ...
          const opacity = calculateOpacity(distance, Math.max(halfWidth, halfHeight))
          cells.push({ x, y, opacity, distance })
        }
      }
    }
