...
export function useGrid(props: BasicCanvasProps) {
  ...
  const drawGrid = (
    ...
    const startX = Math.max(0, Math.floor(-viewportX / cellWidth));
    const startY = Math.max(0, Math.floor(-viewportY / cellHeight));
    const endX = Math.min(
      props.width,
    ...
    const endY = Math.min(
      props.height,
      Math.ceil((canvasHeight - viewportY) / cellHeight) + 1,
    );
    ...
    for (let x = startX; x <= endX; x++) {
      ...
      if (pixelX >= 0 && pixelX <= canvasWidth) {
        ctx.moveTo(pixelX, Math.max(0, viewportY));
        ctx.lineTo(
          pixelX,
          Math.min(canvasHeight, props.height * cellHeight + viewportY),
        );
      }
    }
    ...
    for (let y = startY; y <= endY; y++) {
      const pixelY = Math.round(y * cellHeight + viewportY);
      if (pixelY >= 0 && pixelY <= canvasHeight) {
        ctx.moveTo(Math.max(0, viewportX), pixelY);
        ctx.lineTo(
          Math.min(canvasWidth, props.width * cellWidth + viewportX),
          pixelY,
        );
      }
    }
    ...
    if (gridVisibility.value.showSubGrid) {
      drawSubGrid(ctx, canvasWidth, canvasHeight, viewportX, viewportY);
    }
  };
  ...
  const drawSubGrid = (
    ...
    // 子网格间距（每5个格子一条子网格线）
    const subGridSpacingX = cellWidth * 5;
    const subGridSpacingY = cellHeight * 5;

    ctx.save();
    ctx.globalAlpha = config.subGridOpacity;
    ctx.strokeStyle = config.subGridColor;
    ctx.lineWidth = config.subGridLineWidth;

    // 绘制子网格垂直线
    ctx.beginPath();
    for (let x = 0; x * subGridSpacingX <= props.width * cellWidth; x++) {
      const pixelX = x * subGridSpacingX + viewportX;
      if (pixelX >= 0 && pixelX <= canvasWidth) {
        ctx.moveTo(pixelX, Math.max(0, viewportY));
        ctx.lineTo(
          pixelX,
    ...
    // 绘制子网格水平线
    for (let y = 0; y * subGridSpacingY <= props.height * cellHeight; y++) {
      const pixelY = y * subGridSpacingY + viewportY;
      if (pixelY >= 0 && pixelY <= canvasHeight) {
        ctx.moveTo(Math.max(0, viewportX), pixelY);
        ctx.lineTo(
          Math.min(canvasWidth, props.width * cellWidth + viewportX),
  ...
  const getVisibleGridRange = (
    ...
    return {
      ...
      endX: Math.min(
        props.width,
        Math.ceil((canvasWidth - viewportX) / cellWidth),
      ),
      endY: Math.min(
        props.height,
        Math.ceil((canvasHeight - viewportY) / cellHeight),
      ),
    };
  };
