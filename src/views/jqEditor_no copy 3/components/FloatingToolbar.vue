    ...
    <div class="toolbar-header">
      <div class="toolbar-title">
        ...
        <div class="toolbar-controls">
          ...
          <button class="control-btn close-btn" title="关闭" @click="$emit('close')">
      ...
      <div class="tool-section color-section">
        ...
        <div class="current-color-display">
          ...
          <input
            ...
            :value="colorSystem.currentColor.value"
            type="color"
            class="color-input"
            @input="onColorChange"
          />
        ...
        <!-- 最近使用的颜色 -->
        <div v-if="colorSystem.recentColors.value.length > 0" class="recent-colors">
      ...
      <div class="tool-section brush-section">
        ...
        <div class="brush-size-control">
          <label class="control-label">
            大小: {{ brushSystem.brushConfig.value.sizeCm }}cm
          </label>
          <input
              :value="brushSystem.brushConfig.value.sizeCm"
               type="range"
               :min="0.1"
               :max="2"
               :step="0.1"
          ...
          />
          <div class="size-info">{{ brushSystem.brushMetrics.value.widthInGrids }} 格子</div>
        </div>
        ...
        <div class="brush-shape-control">
          ...
          <div class="shape-buttons">
            <button
              ...
              :key="shape"
              class="shape-btn"
              :class="{ active: shape === brushSystem.brushConfig.value.shape }"
              :title="shape === 'circle' ? '圆形' : '方形'"
              @click="onBrushShapeChange(shape)"
            >
        ...
        <div class="brush-hardness-control">
          <label class="control-label"> 硬度: {{ brushSystem.brushConfig.value.hardness }}% </label>
          <input
            :value="brushSystem.brushConfig.value.hardness"
            type="range"
            min="0"
            max="100"
            class="hardness-slider"
            @input="onBrushHardnessChange"
          />
        </div>
        ...
        <div class="brush-info">
          <div class="info-item">
            ...
            <span class="info-value">
              {{ brushSystem.brushConfig.value.sizeCm }}cm
            </span>
          </div>
          <div class="info-item">
            <span class="info-label">影响格子:</span>
            <span class="info-value">
              {{ brushSystem.brushMetrics.value.widthInGrids * brushSystem.brushMetrics.value.heightInGrids }}
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="tool-section options-section">
        ...
        <div class="option-control">
          <label class="checkbox-label">
            <input :checked="gridSystem.showGrid.value" type="checkbox" class="option-checkbox" @change="onGridToggle" />
        ...
        <!-- 清空按钮 -->
        <div class="option-control">
          <button class="action-btn clear-btn" title="清空画布" @click="$emit('clear-canvas')">
        ...
        <!-- 图片上传按钮 -->
        <div class="option-control">
          <button class="action-btn upload-btn" title="上传图片" @click="triggerImageUpload">
        ...
        <!-- 导出按钮 -->
        <div class="option-control">
          <button class="action-btn export-btn" title="导出图片" @click="$emit('export-image')">
      ...
      <div class="tool-section info-section">
        ...
        <div class="canvas-info">
          ...
          <div class="info-item">
            ...
            <span class="info-value"> {{ props.actualWidth }}×{{ props.actualHeight }}cm </span>
          </div>
          <div class="info-item">
...
interface Props {
  ...
  colorSystem: {
    currentColor: { value: string }
    presetColors: { value: string[] }
    recentColors: { value: string[] }
    customColors: { value: string[] }
  }
  brushSystem: {
    brushConfig: { value: { sizeCm: number; hardness: number; shape: string } }
    brushMetrics: { value: { widthInGrids: number; heightInGrids: number } }
  }
  gridSystem: {
    showGrid: { value: boolean }
    gridData: { value: { cellWidth: number; cellHeight: number } }
  }
...
const emit = defineEmits<{
  ...
  'brush-hardness-change': [hardness: number]
  'brush-shape-change': [shape: string]
  'grid-toggle': [show: boolean]
  close: []
}>()
...
const onBrushSizeChange = (event: Event) => {
  const target = event.target as HTMLInputElement
  emit('brush-size-change', Number(target.value))
}

/**
 * 画笔硬度改变事件
 */
const onBrushHardnessChange = (event: Event) => {
  const target = event.target as HTMLInputElement
  emit('brush-hardness-change', Number(target.value))
}

/**
 * 画笔形状改变事件
 */
const onBrushShapeChange = (shape: string) => {
  emit('brush-shape-change', shape)
}
...
const onGridToggle = (event: Event) => {
  const target = event.target as HTMLInputElement
  emit('grid-toggle', target.checked)
}
