<template>
  <div class="basic-canvas-container">
    ...
    >
      ...
      <!-- 工具栏 -->
      <ToolBar
        v-if="canvas.canvasState.currentTool"
...
import type { BasicCanvasProps, BasicCanvasEmits, GridCoordinate } from './types/canvas'
    ...
    default: {
      ...
      if (num >= 1 && num <= 9) {
        ...
        if (defaultColors[num - 1]) {
          canvas.colorSystem.setColor(defaultColors[num - 1])
          event.preventDefault()
        }
      }
      break
 ...
 */
const getToolCursor = () => {
  const tool = TOOLS.find((t) => t.id === canvas.canvasState.currentTool)
...
const handleToolChange = (tool: ToolType) => {
  ...
  // 如果正在绘制，取消绘制状态
  if (canvas.canvasState.isDrawing) {
    canvas.canvasState.isDrawing = false
    canvas.canvasState.lastDrawnCell = undefined
  }
}
...
defineExpose({
  ...
  setColor: (color: string) => canvas.colorSystem.setColor(color),
