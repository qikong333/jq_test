import type { GridCell, GridStats } from '../types/canvas';
import type { GridStats as GridStatType } from '../types/grid';
...
export class CompressedGridStorage {
  ...
  getCellsInRegion(
    ...
    for (
      ...
      x <= Math.min(this.width - 1, endX);
      x++
    ) {
      for (
        let y = Math.max(0, startY);
        y <= Math.min(this.height - 1, endY);
        y++
      ) {
    ...
    return {
      ...
      hitRate:
        this.stats.totalGets > 0
          ? this.stats.hitCount / this.stats.totalGets
          : 0,
