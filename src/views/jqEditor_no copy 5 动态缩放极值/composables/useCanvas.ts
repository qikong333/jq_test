...
export function useCanvas(
  ...
  const drawGridCells = (context: CanvasRenderingContext2D) => {
    ...
    paintedCells.forEach((cell) => {
      if (cell.color) {
        // 使用Canvas状态缓存设置颜色
        canvasStateCache.applyState(context, { fillStyle: cell.color });
        const x =
          cell.x * cellWidth * viewportSystem.viewportState.zoom +
          viewportSystem.viewportState.pan.x
        const y =
          cell.y * cellHeight * viewportSystem.viewportState.zoom +
          viewportSystem.viewportState.pan.y
