...
/**
  ...
  try {
    ...
    } as { target: { files: File[] } }

    // 配置上传选项
    const options: ImageUploadOptions = {
      maxWidth: Math.max(props.width || 400, 200),
      maxHeight: Math.max(props.height || 800, 200),
      autoResize: true,
      quality: 0.8
    }
    ...
    if (result.success) {
      ...
      // 如果启用自动调整尺寸，更新画布尺寸
      if (options.autoResize && (result.width !== props.width || result.height !== props.height)) {
        // 发出尺寸变化事件，让父组件处理
        emit('size-changed', {
          width: result.width,
      ...
      for (const [key, color] of result.gridData) {
        ...
        if (x >= 0 && x < (props.width || 400) && y >= 0 && y < (props.height || 800)) {
      ...
      console.log('图片上传成功:', {
        ...
        颜色数量: result.colors.length,
        像素数量: result.gridData.size
      })
    } else {
