 ...
 * 负责网格计算、显示控制和渲染配置
 */
export function useGridSystem(props: GridComponentProps): UseGridSystemReturn {
  // 响应式状态
  ...
  const drawGrid = (
    ...
    const endX = Math.min(
      props.width ?? 400,
      Math.ceil((canvasWidth - viewportState.pan.x) / scaledCellWidth) + 1,
    )
    const endY = Math.min(
      props.height ?? 800,
    ...
    for (let x = startX; x <= endX; x++) {
      ...
      if (pixelX >= 0 && pixelX <= canvasWidth) {
        ...
        ctx.lineTo(
          pixelX,
          Math.min(canvasHeight, (props.height ?? 800) * scaledCellHeight + viewportState.pan.y),
        )
      }
    }
    ...
    for (let y = startY; y <= endY; y++) {
      ...
      if (pixelY >= 0 && pixelY <= canvasHeight) {
        ...
        ctx.lineTo(
          Math.min(canvasWidth, (props.width ?? 400) * scaledCellWidth + viewportState.pan.x),
          pixelY,
        )
      }
    }
  ...
  const drawSubGrid = (
    ...
    for (
      let x = 0;
      x * subGridSpacingX <= (props.width ?? 400) * cellWidth * viewportState.zoom;
      x++
    ) {
      ...
      if (pixelX >= 0 && pixelX <= canvasWidth) {
        ...
        ctx.lineTo(
          ...
          Math.min(
            canvasHeight,
            (props.height ?? 800) * cellHeight * viewportState.zoom + viewportState.pan.y,
          ),
        )
      }
    }
    ...
    for (
      let y = 0;
      y * subGridSpacingY <= (props.height ?? 800) * cellHeight * viewportState.zoom;
      y++
    ) {
      ...
      if (pixelY >= 0 && pixelY <= canvasHeight) {
        ...
        ctx.lineTo(
          Math.min(
            canvasWidth,
            (props.width ?? 400) * cellWidth * viewportState.zoom + viewportState.pan.x,
          ),
          pixelY,
        )
      }
